-- Complete Digital Forensics Hub Database Schema
-- This file combines all migration files into one comprehensive schema
...
-- Create enum types for suspect status and involvement level
DO $$ BEGIN
...
-- Add foreign key constraint for forensic_actions template_id
ALTER TABLE public.forensic_actions 
ADD CONSTRAINT fk_forensic_actions_template_id 
FOREIGN KEY (template_id) REFERENCES public.action_templates(id) ON DELETE SET NULL;
